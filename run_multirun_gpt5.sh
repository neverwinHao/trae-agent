#!/bin/bash
# Run naive trae-agent 5 times for GPT-5, then eval each run
# Output: results/multi-run/gpt5/

set -euo pipefail
cd /home/v-haoliu3/SWT_Models/trae-agent
source .venv/bin/activate

OUTPUT_DIR="results/multi-run/gpt5"
MODEL_CONFIG="test_config-gpt5.yaml"
EVAL_BASE="/home/v-haoliu3/swt-bench-verified/swt-bench"
INFERENCE_DIR="${EVAL_BASE}/inference_output/multi-run/gpt5"
EVAL_RESULTS_DIR="${EVAL_BASE}/evaluation_results/multi-run/gpt5"
mkdir -p "${INFERENCE_DIR}" "${EVAL_RESULTS_DIR}"

for i in $(seq 1 5); do
    RUN_ID="gpt5-naive-run${i}"
    ...
    python -m evaluation.run_evaluation \
        ...
        --working-dir "./trae-workspace-gpt5"
